...
create table if not exists services (id uuid default gen_random_uuid() primary key, provider_service_id text unique not null, name text not null, category text, platform text, type text, rate numeric(14,6) default 0 not null, min_quantity integer default 1 not null, max_quantity integer default 100000 not null, description text, is_active boolean default true not null, provider_rate numeric(14,8) default 0 not null, average_time integer, created_at timestamptz default now() not null);
alter table if exists services add column if not exists average_time integer;
alter table if exists services alter column rate type numeric(14,6) using rate::numeric(14,6);
alter table if exists services alter column provider_rate type numeric(14,8) using provider_rate::numeric(14,8);
